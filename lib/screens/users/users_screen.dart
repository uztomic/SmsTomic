    ...
    return Scaffold(
      body: BlocConsumer<UserCubit, UserState>(
        ...
        builder: (context, state) {
          ...
          final currentUid = (context.read<AuthBloc>().state as AuthAuthenticated).user.uid;
              ...
              return Padding(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: user.isAdmin ? AppColors.rose.withValues(alpha: 0.3) : AppColors.roseLight,
                    ),
                  ),
                  child: Column(
                    ...
                    children: [
                      Row(
                        children: [
                          CircleAvatar(
                            backgroundColor: user.isAdmin ? AppColors.rose : AppColors.roseLight,
                            child: Icon(
                              user.isAdmin ? Icons.shield_rounded : Icons.person_rounded,
                              color: user.isAdmin ? Colors.white : AppColors.roseDark,
                            ),
                          ),
                      ...
                      Wrap(
                        ...
                        children: user.isAdmin
                            ? [Chip(label: const Text('To\'liq huquq'), backgroundColor: AppColors.rose.withValues(alpha: 0.15))]
                            : user.permissions.isEmpty
                                ? [const Chip(label: Text('Ruxsat yo\'q'))]
                                : user.permissions.map((p) => Chip(label: Text(p.label))).toList(),
                      ),
                    ],
                  ),
                ),
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          FilledButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Tasdiqlash')),
        ],
      ),
    );
